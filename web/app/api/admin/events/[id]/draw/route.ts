...
export async function POST(
  ...
  const { data: registrations } = await supabase
    .from('red_book')
    .select('member_id, members(id, first_name, last_name, games_played, times_as_booker, last_booker_date, first_tee_count, tenth_tee_count, suspended, does_not_book)')
    .eq('event_id', eventId)

  const members = (registrations ?? [])
    .map(r => r.members as { id: string; first_name: string; last_name: string; games_played: number; times_as_booker: number; last_booker_date: string | null; first_tee_count: number; tenth_tee_count: number; suspended: boolean; does_not_book: boolean } | null)
    .filter((m): m is NonNullable<typeof m> => m !== null && !m.suspended)
